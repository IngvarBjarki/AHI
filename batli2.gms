...
Ex2(t).. EXCECUTIVE_OVERVIEW('DPC',t) =e= sum((n,i), h(n,i)*r(n,i,t)) + sum(j, y(j,t)*c(j)/1000);
Ex3(t).. EXCECUTIVE_OVERVIEW('SP',t) =e=  (sum((k,j), (GAMMA(j,k)/1000) * sum(l, q(l,j)*u(l,j,k,t)))
            - sum((k,j), (DELTA(j,k)/(1000*1000)) * sum(l, q(l,j)*q(l,j) * u(l,j,k,t))/power(demand_growth(j), ord(t)-1)));
Ex4(t).. EXCECUTIVE_OVERVIEW('FC',t) =e=  sum(m, Cap(m,t)*FCost(m)/1000) ;
